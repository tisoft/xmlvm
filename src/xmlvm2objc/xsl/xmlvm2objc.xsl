...
int main(int argc, char* argv[])
{
    ...
    <xsl:value-of select="replace($cl/@package, '\.', '_')"/>
    <xsl:text>_</xsl:text>
...
<xsl:template name="emitInterfaces">
  <xsl:for-each select="vm:class">
    <xsl:text>
@interface </xsl:text>
    <xsl:value-of select="replace(@package, '\.', '_')"/>
    <xsl:text>_</xsl:text>
    <xsl:value-of select="@name"/>
    <xsl:text> : </xsl:text>
    <xsl:value-of select="replace(@extends, '\.', '_')"/>
    <xsl:text> {
</xsl:text>
...
<xsl:template match="jvm:ldc|jvm:ldc2_w">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:message select="'Unkown type in jvm:ldc'"/>
...
<xsl:template match="jvm:getstatic">
  ...
  <xsl:text> = [</xsl:text>
  <xsl:value-of select="replace(@class-type, '\.', '_')"/>
  <xsl:text> _GET_STATIC_</xsl:text>
  <xsl:value-of select="replace(@class-type, '\.', '_')"/>
  <xsl:text>_</xsl:text>
  <xsl:value-of select="@field"/>
...
<xsl:template name="appendSignature">
  <xsl:choose>
    <xsl:when test="count(vm:signature/vm:parameter) != 0">
      ...
      <xsl:for-each select="vm:signature/vm:parameter">
        <xsl:text>_</xsl:text>
        <xsl:value-of select="replace(replace(@type, '\.', '_'), '\[\]', '_ARRAYTYPE')"/>
      </xsl:for-each>
    </xsl:when>
  </xsl:choose>
</xsl:template>
...
<xsl:function name="vm:isObjectRef" as="xs:boolean">
  ...
  <xsl:value-of select="not($type='int' or $type='float' or $type='double' or
