...
<xsl:template match="clr:stloc[dfa:stack-pre/dfa:elem[last()][@type = 'int' or @type = 'byte' or @type = 'short']]">
  <jvm:istore>
    <xsl:variable name="this-offset"
                  select="if (../../@isStatic = 'true') then 0 else 1"/>
    <xsl:variable name="offset">
...
<xsl:template match="clr:conv.i4[dfa:stack-pre/dfa:elem[last()][@type = 'byte' or @type = 'short']]">
  <!--  do nothing --> 
</xsl:template>
