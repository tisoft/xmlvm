...
public class ExeInputProcess extends InputProcess<ExeFile> {
    ...
    public boolean process() {
        ...
        try {
            ...
            String className = input.getFile().getName().substring(0,
                    input.getFile().getName().length() - ExeFile.EXE_ENDING.length());
            xmlvmResource = new XmlvmResource(className, Type.CLI, xmlvmDoc);
            return true;
        ...
        for (int i = 0; i < td.length; i++) {
            ...
            if (flags != 0) {
                ...
                for (int j = 0; j < nestedClasses.length; j++) {
                    this.nestedClassNamespaceMap.put(nestedClasses[j].getName(), nestedNamespace);
                }
            }
        }
