...
public class InstructionProcessor {
    ...
    static Pattern        match3Op          = Pattern
                                                    .compile("^"
                                                            + "(add|sub|mul|div|rem|and|or|xor|shl|shr|ushr)-"
                                                            + nonObjTypesString + ".*");
    static Pattern        match2Op          = Pattern
                                                    .compile("^"
                                                            + "(add|sub|mul|div|rem|and|or|xor|shl|shr|ushr)-"
                                                            + nonObjTypesString + "-2addr" + ".*");
    static Pattern        matchOneOp        = Pattern.compile("^"
                                                    + "(add|rsub|mul|div|rem|and|or|xor)-"
                                                    + nonObjTypesString + "-lit(8|16)" + ".*");
    ...
    static Pattern        matchIf2          = Pattern.compile("^" + "(if)-(eq|ne|lt|ge|gt|le)"
                                                    + ".*");
    static Pattern        iput              = Pattern.compile("^" + "(iput|sput)-"
                                                    + nonObjTypesString + ".*");
    static Pattern        iget              = Pattern.compile("^" + "(iget|sget)-"
                                                    + nonObjTypesString + ".*");
    ...
    static Pattern        agetNonObj        = Pattern.compile("^" + "aget-" + nonObjTypesString
                                                    + ".*");
    static Pattern        conversionNegNot  = Pattern.compile("^" + "(neg|not)-"
                                                    + nonObjTypesString + ".*");
    static Pattern        conversionfromTo  = Pattern.compile("^" + nonObjTypesString + "-to-"
                                                    + nonObjTypesString + ".*");
     ...
     */
    static boolean processGeneric(Element element, InstructionUseInfo i)
        ...
        if (matchOneOp.matcher(elemName).matches()) {
            ...
            i.isWrite = true;
        } else if (conversionNegNot.matcher(elemName).matches()
                || conversionfromTo.matcher(elemName).matches()) {
            i.isWrite = true;
    ...
    static public void process_move_object_from16(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
    ...
    static public void process_aput(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = false;
        i.requiresRetain.orEq(getDestReg(element, "vx").and(i.usesAsObj()));
    }

    static public void process_aget(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain.orEq(i.writesObj());
    ...
    static public void process_aget_object(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain.orEq(i.writesObj());
    ...
    static public void process_iget_object(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain.orEq(i.writesObj());
    ...
    static public void process_sget_object(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain.orEq(i.writesObj());
    }
    ...
    static public void process_sget(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain = i.writesObj();
    }
    ...
    static public void process_iget(Element element, InstructionUseInfo i)
            throws DataConversionException {
        i.isWrite = true;
        i.requiresRetain = i.writesObj();
    }
    ...
    static public void process_filled_new_array_range(Element element, InstructionUseInfo i)
            throws DataConversionException {
