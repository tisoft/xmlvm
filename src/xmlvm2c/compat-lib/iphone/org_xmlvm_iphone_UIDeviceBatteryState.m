...
{
    staticInitializerLock(&__TIB_org_xmlvm_iphone_UIDeviceBatteryState);

    // While the static initializer mutex is locked, locally store the value of
    // whether class initialization began or not
    int initBegan = __TIB_org_xmlvm_iphone_UIDeviceBatteryState.classInitializationBegan;

    // Whether or not class initialization had already began, it has begun now
    __TIB_org_xmlvm_iphone_UIDeviceBatteryState.classInitializationBegan = 1;

    staticInitializerUnlock(&__TIB_org_xmlvm_iphone_UIDeviceBatteryState);

    JAVA_LONG curThreadId = (JAVA_LONG)pthread_self();
    if (initBegan) {
        if (__TIB_org_xmlvm_iphone_UIDeviceBatteryState.initializerThreadId != curThreadId) {
            // Busy wait until the other thread finishes initializing this class
            while (!__TIB_org_xmlvm_iphone_UIDeviceBatteryState.classInitialized) {
                // do nothing
            }
        }
    } else {
...
void __INIT_INSTANCE_MEMBERS_org_xmlvm_iphone_UIDeviceBatteryState(JAVA_OBJECT me, int derivedClassWillRegisterFinalizer)
{
    __INIT_INSTANCE_MEMBERS_java_lang_Object(me, 0 || derivedClassWillRegisterFinalizer);
    //XMLVM_BEGIN_WRAPPER[__INIT_INSTANCE_MEMBERS_org_xmlvm_iphone_UIDeviceBatteryState]
    //XMLVM_END_WRAPPER
}
