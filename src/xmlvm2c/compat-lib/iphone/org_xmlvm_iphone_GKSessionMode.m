...
{
    staticInitializerLock(&__TIB_org_xmlvm_iphone_GKSessionMode);

    // While the static initializer mutex is locked, locally store the value of
    // whether class initialization began or not
    int initBegan = __TIB_org_xmlvm_iphone_GKSessionMode.classInitializationBegan;

    // Whether or not class initialization had already began, it has begun now
    __TIB_org_xmlvm_iphone_GKSessionMode.classInitializationBegan = 1;

    staticInitializerUnlock(&__TIB_org_xmlvm_iphone_GKSessionMode);

    JAVA_LONG curThreadId = (JAVA_LONG)pthread_self();
    if (initBegan) {
        if (__TIB_org_xmlvm_iphone_GKSessionMode.initializerThreadId != curThreadId) {
            // Busy wait until the other thread finishes initializing this class
            while (!__TIB_org_xmlvm_iphone_GKSessionMode.classInitialized) {
                // do nothing
            }
        }
    } else {
...
void __INIT_IMPL_org_xmlvm_iphone_GKSessionMode()
{
    ...
    __TIB_org_xmlvm_iphone_GKSessionMode.baseType = JAVA_NULL;
    __CLASS_org_xmlvm_iphone_GKSessionMode_1ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_GKSessionMode);
    __CLASS_org_xmlvm_iphone_GKSessionMode_2ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_GKSessionMode_1ARRAY);
    __CLASS_org_xmlvm_iphone_GKSessionMode_3ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_GKSessionMode_2ARRAY);
...
void __INIT_INSTANCE_MEMBERS_org_xmlvm_iphone_GKSessionMode(JAVA_OBJECT me, int derivedClassWillRegisterFinalizer)
{
    ...
    //XMLVM_BEGIN_WRAPPER[__INIT_INSTANCE_MEMBERS_org_xmlvm_iphone_GKSessionMode]
    //XMLVM_END_WRAPPER
}
